...
const FALLBACK_IMAGE = 'https://images.unsplash.com/photo-1499750310107-5fef28a66643?w=600';
...
const inputBase = {
  border: 'none', outline: 'none', fontSize: 14, background: 'transparent', flex: 1
};

const selectBase = {
  padding: '10px 16px', background: 'white', border: '1px solid #e2e8f0',
  borderRadius: 10, fontSize: 14, cursor: 'pointer', outline: 'none', minWidth: 160
};
      ...
      <div style={{ minHeight: '100vh', background: '#f8fafc' }}>
        <section style={{ background: `linear-gradient(135deg, ${blueDark}, #1e3a5f)`, padding: '60px 20px 40px' }}>
          <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center', color: 'white' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.15)', padding: '10px 20px', borderRadius: 20, fontSize: 14, fontWeight: 500, marginBottom: 24 }}>
              <Sparkles size={20} /> CERESENSE INSIGHTS
            </div>
            <h1 style={{ fontSize: 42, fontWeight: 800, marginBottom: 16 }}>Tech Insights & Innovations</h1>
            <p style={{ fontSize: 18, opacity: 0.9 }}>Loading articles...</p>
          </div>
        </section>
      ...
      <section style={{ background: `linear-gradient(135deg, ${blueDark}, #1e3a5f)`, padding: '60px 20px 40px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center', color: 'white' }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 style={{ fontSize: 42, fontWeight: 800, lineHeight: 1.2, marginBottom: 16 }}>Tech Insights & Innovations</h1>
            <p style={{ fontSize: 18, lineHeight: 1.6, opacity: 0.9, maxWidth: 600, margin: '0 auto' }}>Expert perspectives on technology, education, and the future of learning</p>
          </motion.div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          {error && (
            <div style={{ textAlign: 'center', padding: 20, background: '#fef2f2', borderRadius: 12, color: '#dc2626', marginBottom: 24 }}>
              ...
              <button onClick={onRetry} style={{ padding: '8px 20px', background: blue, color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600 }}>Try Again</button>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 16, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ flex: 1, minWidth: 250, display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', background: 'white', border: '1px solid #e2e8f0', borderRadius: 10 }}>
              ...
              <input type="text" placeholder="Search articles..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={inputBase} />
              {searchTerm && <button onClick={() => setSearchTerm('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', fontSize: 16 }}>✕</button>}
            </div>
            <select value={selectedTag} onChange={e => setSelectedTag(e.target.value)} style={selectBase}>
            ...
            {filteredPosts.map((post, index) => (
              ...
              >
                <div style={{ position: 'relative', height: 200, overflow: 'hidden', background: '#f1f5f9' }}>
                  <img src={getImageUrl(post.cover_image || post.coverImage)} alt={post.title || 'Blog post'} style={{ width: '100%', height: '100%', objectFit: 'cover' }}
...
const BlogDetailPage = ({ postId, onBack, allPosts }) => {
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 400, background: '#f8fafc' }}>
  ...
  if (!post) {
    return (
      <div style={{ padding: 60, textAlign: 'center', background: '#f8fafc', minHeight: '100vh' }}>
        <h3 style={{ color: '#1e293b' }}>Article not found</h3>
        <button onClick={onBack} style={{ padding: '12px 24px', background: blue, color: 'white', border: 'none', borderRadius: 10, cursor: 'pointer', marginTop: 16, fontWeight: 600 }}>
          ← Back to Articles
        </button>
      </div>
    );
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 20px', display: 'grid', gridTemplateColumns: '1fr 340px', gap: 30 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: 'white', borderRadius: 14, padding: 20, border: '1px solid #e2e8f0' }}>
            <h4 style={{ fontSize: 15, fontWeight: 700, color: '#1e293b', margin: '0 0 12px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
              <Search size={18} color={blue} /> Search
            </h4>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10 }}>
              ...
              <input type="text" placeholder="Search articles..." value={sidebarSearch} onChange={e => setSidebarSearch(e.target.value)} style={inputBase} />
            </div>
            {sidebarSearch && filteredSidebarPosts.length > 0 && (
              <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {filteredSidebarPosts.map(p => (
                  <div key={p.id} onClick={onBack}
                    style={{ padding: '8px 10px', borderRadius: 8, cursor: 'pointer', fontSize: 13, color: '#1e293b', fontWeight: 500, transition: 'background 0.15s' }}
          ...
          <div style={{ background: 'white', borderRadius: 14, padding: 20, border: '1px solid #e2e8f0' }}>
            <h4 style={{ fontSize: 15, fontWeight: 700, color: '#1e293b', margin: '0 0 12px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
              <Share2 size={18} color={blue} /> Share
            </h4>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {[
                { label: 'Facebook', color: '#1877f2', platform: 'facebook' },
                { label: 'Twitter', color: '#1da1f2', platform: 'twitter' },
                { label: 'LinkedIn', color: '#0a66c2', platform: 'linkedin' },
              ].map(btn => (
                <button key={btn.platform} onClick={() => handleShare(btn.platform)} style={shareBtnStyle(btn.color)}>
                  <ExternalLink size={14} /> {btn.label}
                </button>
              ))}
              <button onClick={() => handleShare('copy')} style={shareBtnStyle(copied ? '#16a34a' : '#64748b')}>
                {copied ? <Copy size={14} /> : <Link size={14} />} {copied ? 'Copied!' : 'Copy'}
              </button>
            </div>
          </div>
          ...
          {relatedPosts.length > 0 && (
            <div style={{ background: 'white', borderRadius: 14, padding: 20, border: '1px solid #e2e8f0' }}>
              <h4 style={{ fontSize: 15, fontWeight: 700, color: '#1e293b', margin: '0 0 12px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
                <Bookmark size={18} color={blue} /> Related
              </h4>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {relatedPosts.map(p => (
                  <div key={p.id} onClick={onBack}
                    style={{ display: 'flex', gap: 10, cursor: 'pointer', padding: '8px 10px', borderRadius: 10, transition: 'background 0.15s' }}
          ...
          <div style={{ background: `linear-gradient(135deg, ${blue}, ${blueDark})`, borderRadius: 14, padding: 24, color: 'white', textAlign: 'center' }}>
            ...
            <h4 style={{ fontSize: 16, fontWeight: 700, margin: '0 0 4px 0' }}>Enjoy this?</h4>
            <p style={{ fontSize: 13, opacity: 0.85, margin: '0 0 16px 0' }}>Share with your network!</p>
